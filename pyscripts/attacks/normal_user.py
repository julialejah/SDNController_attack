...
def generate_ssh_traffic(tiempo_deseadofuncion, dirIP, intfc):
...
def getHttp(dst_ip,intfc):
    ...
    # Enviar el paquete
    print("Get http")
    sendp(http_request1, iface = intfc)

def postHttp(dst_ip,intfc):
...
def generate_http_traffic(tiempo_deseadofuncion, intfc):
    # Tiempo de inicio
    tiempo_iniciofuncion = time.time()
    ...
    while time.time() - tiempo_iniciofuncion < tiempo_deseadofuncion: 
        ...
        #ip
        dst_ip = random.choice(rangoIP)
        # Llamar a la función elegida
        funcion_elegida(dst_ip,intfc)
        # time.sleep(10)
#        espera = random.randint(1, 3)
#        time.sleep(espera) 
        #filtro http
...
def generate_https_traffic(tiempo_deseadofuncion, intfc):
...
def generate_mail_traffic(tiempo_deseadofuncion,intfc):
    tiempo_iniciofuncion = time.time()
...
#        time.sleep(espera) 
        #envia paquete
        scapy.all.sendp(mail_request, iface = intfc)
        #filtro smtp

def generate_ftp_traffic(tiempo_deseadofuncion,intfc):
    tiempo_iniciofuncion = time.time()
    ...
    while time.time() - tiempo_iniciofuncion < tiempo_deseadofuncion:
        ...
        scapy.all.sendp(ftp_request, iface = intfc)
        # tiempo de espera
#        espera = random.randint(1, 3)
#        time.sleep(espera)
        #filtro ftp

def generate_traffic(sshIP,intfc):
    # Generar tráfico aleatorio
    traffic_type = random.randint(1, 5)
    # Tiempo que dura el trafico
    traffic_time = random.randint(1, 3) #entre 10 segundos y 3,333 minutos
    print("Tiempo de trafico:", traffic_time)
    if traffic_type == 1:
        print("HTTP")
        generate_http_traffic(traffic_time,intfc)            
    elif traffic_type == 2:
        print("HTTPS")
        generate_https_traffic(traffic_time,intfc)
            
    elif traffic_type == 3:
        print("SSH")
        generate_ssh_traffic(traffic_time,sshIP,intfc)
            
    elif traffic_type == 4:
        print("SMTP")
        generate_mail_traffic(traffic_time,intfc)
    elif traffic_type == 5:
        print("FTP")
        generate_ftp_traffic(traffic_time,intfc)
